...
export const StaggeredMenu = ({ position = "right", colors = ["#504f53", "#201f23"], items = [], displayItemNumbering = true, className, logoUrl = "/favicon-628.png", menuButtonColor = "#000000", openMenuButtonColor = "#000000", changeMenuColorOnOpen = true, isFixed = false, accentColor = "#5227FF", closeOnClickAway = true, onMenuOpen, onMenuClose }) => {
      ...
      return (
            <div className={`sm-scope z-40 ${isFixed ? "fixed top-0 left-0 w-screen h-screen overflow-hidden" : "w-full h-full"}`}>
                  <div className={(className ? className + " " : "") + "staggered-menu-wrapper pointer-events-none relative w-full h-full"} style={accentColor ? { ["--sm-accent"]: accentColor } : undefined} data-position={position} data-open={open || undefined}>
                        <div ref={preLayersRef} className="sm-prelayers absolute top-0 right-0 bottom-0 pointer-events-none z-[5]" aria-hidden="true">
                              {(() => {
                                    ...
                                    return arr.map((c, i) => <div key={i} className="sm-prelayer absolute top-0 right-0 h-full w-full translate-x-0" style={{ background: c }} />);
                              })()}
                        </div>
                              ...
                              <button ref={toggleBtnRef} className="sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer text-[#e9e9ef] font-medium leading-none overflow-visible pointer-events-auto" aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open} aria-controls="staggered-menu-panel" onClick={toggleMenu} type="button">
                                    <span ref={textWrapRef} className="sm-toggle-textWrap relative inline-block h-[1em] overflow-hidden whitespace-nowrap w-[var(--sm-toggle-width,auto)] min-w-[var(--sm-toggle-width,auto)]" aria-hidden="true">
                                          <span ref={textInnerRef} className="sm-toggle-textInner flex flex-col leading-none">
                                                {textLines.map((l, i) => (
                                                      <span className="sm-toggle-line block font-semibold h-[1em] leading-none" key={i}>
                                                            {l}
                                                      </span>
                                                ))}
                                          </span>
                        ...
                        <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel absolute top-0 right-0 h-full pointer-events-auto bg-white flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[12px]" style={{ WebkitBackdropFilter: "blur(12px)" }} aria-hidden={!open}>
                              <div className="sm-panel-inner flex-1 flex flex-col gap-5">
                                          ...
                                          {items.length &&
                                                items.map((it, idx) => (
                                                      <li className="sm-panel-itemWrap cursor-pointer relative overflow-hidden leading-none" key={it.label + idx}>
                                                            <Link onClick={toggleMenu} to={`${it.link}`} className="sm-panel-item relative text-black font-semibold  cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]" aria-label={it.ariaLabel} data-index={idx + 1}>
                                    ...
                                    <ul className="sm-socials-list list-none m-0 p-0 flex flex-row items-center gap-4 flex-wrap" role="list">
                                          <li>
                                                {!status && (
                                                      <Link onClick={toggleMenu} className="border-[0.1px] rounded-full px-3 py-1" to="/create-account ">
                                                            Create account
                                                      </Link>
                                                )}
                                          </li>
                                          <li>
                                                <Link onClick={toggleMenu} className="px-3 py-2 flex items-center justify-center-safe gap-2 border-[1px] rounded-full text-[var(--color-wht)] bg-[var(--color-bl)]  border-white/60" to={`${status ? "/write-post" : "/login"}`}>
                                                      {status ? <LuSquarePen /> : <MdLogin />}
                                                      <p className="leading-none whitespace-nowrap tracking-tight">{status ? "Write Blog" : "Login"}</p>
                                                </Link>
                                          </li>
                                          <li onClick={toggleMenu}> {status && <Logout />}</li>
                                    </ul>
                              </div>
                        </aside>
                  </div>
